...
A comprehensive single-cell RNA-seq analysis framework with **4 mandatory QC modules + 41 optional analysis modules (45 total) + automatic dependency resolution + GPU acceleration + categorized output**. Structural registration is not a scientific production certificate: every module retains its own evidence and release status. Module counts are asserted against `workflow/modular/module_catalog.py` by `tests/test_readme_module_counts.py` — see [Historical: Wave 4/5 Progress Narrative](#historical-wave-45-progress-narrative-2026-04-26--2026-05-16) for the superseded Wave 4/5 reproducibility snapshots.
...
### scATAC pseudobulk DA (staged, non-production)

`scatac_pseudobulk_da` is structurally registered but remains
`staged_nonproduction_missing_representative_real_data` and always emits
`claimable: false`. It aggregates sparse cells-by-peaks **raw integer counts**
by named biological sample and named cell group; cells are never replicates.
`atac_ingest` emits an ordered peak-axis certificate, and the DA module rejects
missing, reordered, duplicated, blank, or index-fallback peak identities.

Confirmatory mode requires explicit sample, group, peak-ID, condition, test,
and reference fields; exactly the declared two condition levels; and at least
two biological samples per condition within every analyzed group. It uses
DESeq2 as the primary engine with a mandatory edgeR QL cross-check in the
declared `r_multiomics` environment. Aggregation-only mode exports pseudobulk
counts and design metadata only; it must not be described as differential
accessibility.
...
the implementation contract only and do not promote a biological claim.
...
Reference H5AD files materialized via `scripts/materialize_cellxgene_reference.py` are loaded directly by the compute lanes and can be rechecked offline with `--verify-only`. Reference mapping defaults to `auto`, but `auto` does not benchmark CPU and GPU on a production input. It selects exactly one backend from `ops/policy/gpu_backend_validations.json`: only an exact, current promoted certificate may select GPU; revoked/unknown domains, `gpu_mode=off`, missing CUDA/cuML, or backend-version drift go directly to CPU. The `trevino-fetal-cortex-v1` certificate is currently revoked, so `auto` selects CPU before mapping and explicit GPU fails closed. CPU is the conservative fallback, not biological truth. The mapping/OOD contract and runbook are in [`docs/REFERENCE_ATLAS_OOD.md`](docs/REFERENCE_ATLAS_OOD.md) and [PROTOCOL.md](PROTOCOL.md).

The historical run `/home/zerlinshen/projects/reference-atlas-ood-validation/runs/2026-08-22T1558Z-c38053d/` measured `11.04x`, but its source-wide retained-feature mask was not independently query-blind and no longer authorizes routing. The corrected reference-only run `/home/zerlinshen/projects/reference-atlas-ood-validation/runs/2026-08-23T1054Z-0b66858/` failed all four predeclared OOD gates on CPU and the GPU lane additionally failed exact confidence repeatability plus the `rtol=atol=1e-6` distance contract (maximum pairwise drift `2.1905e-6`). This is a negative technical result using proxy labels, not proof that CPU is biological ground truth; it is sufficient to revoke GPU auto-routing for this domain.
...
Permutation-linkage unit tests use a bounded, planted synthetic projection;
full-cohort runtime and memory claims belong to the staircase benchmark tiers.
...
- **41 optional analysis modules** covering the full scRNA-seq workflow (45 total; see `workflow/modular/module_catalog.py`)
...
| `gene_signature_scoring` | 15 built-in signatures (10 hallmarks + mac/STAB1 panels) + custom JSON | clustering |
...
| **Built-in signatures** | 15 panels (10 cancer hallmarks + macrophage / De Zuani STAB1 programmes; see table below) |
...
| foetal_like_mac | FOLR2, LYVE1, MRC1, SIGLEC1, C1QA, C1QB, SELENOP, RNASE1, F13A1, CD163, MARCO | Mulder-inspired TRM panel (Immunity 2021); secondary to De Zuani panels for NC2024-F3-02 |
| spp1_mac | SPP1, FABP5, TREM2, APOE, CTSB, CTSD, LGALS3, CHIT1 | Companion inflammatory/SPP1+ mac axis |
| de_zuani_stab1_foetal | STAB1, FOLR2, SLC40A1, MERTK, GPR34, F13A1 | De Zuani et al. *Nat Commun* 2024 DOI: [10.1038/s41467-024-48700-8](https://doi.org/10.1038/s41467-024-48700-8) — foetal-typical STAB1 genes named in text |
| de_zuani_stab1_signature_reconstructed | SLC40A1, SELENOP, OLFML3, FCGBP, IGSF21, IL2RA, C3, SIGLEC8, F13A1, ITGA9, CTTNBP2, STAB1, ADGRG6, ADAMDEC1, CXCL12, NCKAP5, ENPP2, SRGAP3, AC079015.1, IGF1 | DEA reconstruction from Supp Data 20/21 (AM∩AIM STAB1-up top-20); ≈Fig 5I, not OCR |
| de_zuani_cholesterol_export | ABCA1, TREM2, APOE, APOC1, SPP1, FABP5 | Tumour mac cholesterol-export markers highlighted in De Zuani 2024 |
...
| 89 | Mulder et al. et al., *Immunity*, 2021 | [10.1016/j.immuni.2021.07.007](https://doi.org/10.1016/j.immuni.2021.07.007) | `gene_signature_scoring` (Cross-tissue monocyte/macrophage programmes; tissue-resident / foetal-like macrophage panels (FOLR2/LYVE1/MRC1-axis) inform the foetal_like_mac builtin. Panel is literature-inspired, not an exact paper gene-list reimplementation.) |
| 90 | Mulder et al., *Immunity*, 2021 | [10.1016/j.immuni.2021.07.007](https://doi.org/10.1016/j.immuni.2021.07.007) | `gene_signature_scoring` (auto-detected from module source) |
| 91 | De Zuani et al. et al., *Nature Communications*, 2024 | [10.1038/s41467-024-48700-8](https://doi.org/10.1038/s41467-024-48700-8) | `gene_signature_scoring` (STAB1+ tumour macrophages with oncofoetal reprogramming, iron export (SLC40A1/ferroportin), and cholesterol-export programmes (ABCA1/TREM2). Paper text names foetal-typical STAB1 signature genes: STAB1, FOLR2, SLC40A1, MERTK, GPR34, F13A1. Full 20-gene STAB1 signature is Fig 5I; de_zuani_stab1_signature_reconstructed is DEA-reconstructed from Supplementary Data 20/21 (AM∩AIM STAB1-up, padj≤0.05, |log2FC|≥1, top-20 by min|log2FC|), not a pixel OCR of Fig 5I.) |
| 92 | De Zuani et al., *Nature Communications*, 2024 | [10.1038/s41467-024-48700-8](https://doi.org/10.1038/s41467-024-48700-8) | `gene_signature_scoring` (auto-detected from module source) |
| 93 | Trevino et al., *Cell*, 2021 | [10.1016/j.cell.2021.07.039](https://doi.org/10.1016/j.cell.2021.07.039) | `gwas_overlay` (auto-detected from module source) |
| 94 | Trevino AE, Müller F, Andersen J, et al. et al., *Cell*, 2021 | [10.1016/j.cell.2021.07.039](https://doi.org/10.1016/j.cell.2021.07.039) | `gwas_overlay` (Source paper for F-7 GWAS overlay reproduction (Wave-5.6 MV3).) |
| 95 | Sollis et al., *Nucleic Acids Research*, 2022 | [10.1093/nar/gkac1010](https://doi.org/10.1093/nar/gkac1010) | `gwas_overlay` (auto-detected from module source) |
| 96 | Sollis E, Mosaku A, Abid A, et al. et al., *Nucleic Acids Research*, 2023 | [10.1093/nar/gkac1010](https://doi.org/10.1093/nar/gkac1010) | `gwas_overlay` (NHGRI-EBI GWAS Catalog: source of SNP-trait associations consumed by this module.) |
| 97 | Lieberman-Aiden et al. et al., *Science*, 2009 | [10.1126/science.1181369](https://doi.org/10.1126/science.1181369) | `hic_ingest` (Original Hi-C methodology — contact-map binning conventions adopted here.) |
| 98 | Lieberman-Aiden et al., *Science*, 2009 | [10.1126/science.1181369](https://doi.org/10.1126/science.1181369) | `hic_ingest` (auto-detected from module source) |
| 99 | Abdennur et al., *Bioinformatics*, 2019 | [10.1093/bioinformatics/btz540](https://doi.org/10.1093/bioinformatics/btz540) | `hic_ingest` (auto-detected from module source) |
| 100 | Abdennur, Mirny et al., *Bioinformatics*, 2020 | [10.1093/bioinformatics/btz540](https://doi.org/10.1093/bioinformatics/btz540) | `hic_ingest` (Canonical .cool / .mcool sparse storage format. This module reads cooler files via the optional `cooler` Python package when installed; falls back to TSV for portability.) |
| 101 | Lieberman-Aiden et al. et al., *Science*, 2009 | [10.1126/science.1181369](https://doi.org/10.1126/science.1181369) | `hic_tad` (Original A/B compartment definition via first eigenvector of the correlation matrix.) |
| 102 | Lieberman-Aiden et al., *Science*, 2009 | [10.1126/science.1181369](https://doi.org/10.1126/science.1181369) | `hic_tad` (auto-detected from module source) |
| 103 | Nora et al. et al., *Nature*, 2012 | [10.1038/nature11049](https://doi.org/10.1038/nature11049) | `hic_tad` (Original TAD identification methodology.) |
| 104 | Nora et al., *Nature*, 2012 | [10.1038/nature11049](https://doi.org/10.1038/nature11049) | `hic_tad` (auto-detected from module source) |
| 105 | Crane et al. et al., *Nature*, 2015 | [10.1038/nature14450](https://doi.org/10.1038/nature14450) | `hic_tad` (Insulation-score TAD boundary detection method implemented here.) |
| 106 | Crane et al., *Nature*, 2015 | [10.1038/nature14450](https://doi.org/10.1038/nature14450) | `hic_tad` (auto-detected from module source) |
| 107 | Tirosh et al. et al., *Science*, 2016 | [10.1126/science.aad0501](https://doi.org/10.1126/science.aad0501) | `immune_phenotyping` (Same scoring methodology scoped to immune cell-type marker sets.) |
| 108 | Tirosh et al., *Science*, 2016 | [10.1126/science.aad0501](https://doi.org/10.1126/science.aad0501) | `immune_phenotyping` (auto-detected from module source) |
| 109 | Zilionis et al., *Immunity*, 2019 | [10.1016/j.immuni.2019.03.009](https://doi.org/10.1016/j.immuni.2019.03.009) | `immune_phenotyping` (auto-detected from module source) |
| 110 | Zilionis et al. et al., *Immunity*, 2019 | [10.1016/j.immuni.2019.03.009](https://doi.org/10.1016/j.immuni.2019.03.009) | `immune_phenotyping` (Canonical lung-cancer immune sub-typing markers underlying many phenotype calls.) |
| 111 | Lopez et al. et al., *Nature Methods*, 2018 | [10.1038/s41592-018-0229-2](https://doi.org/10.1038/s41592-018-0229-2) | `integration_select` (scVI candidate embeddings scored by the seed sweep in this gate.) |
| 112 | Lopez et al., *Nature Methods*, 2018 | [10.1038/s41592-018-0229-2](https://doi.org/10.1038/s41592-018-0229-2) | `integration_select` (auto-detected from module source) |
| 113 | Korsunsky et al. et al., *Nature Methods*, 2019 | [10.1038/s41592-019-0619-0](https://doi.org/10.1038/s41592-019-0619-0) | `integration_select` (Primary candidate backend scored by this gate (Harmony).) |
| 114 | Korsunsky et al., *Nature Methods*, 2019 | [10.1038/s41592-019-0619-0](https://doi.org/10.1038/s41592-019-0619-0) | `integration_select` (auto-detected from module source) |
| 115 | Luecken et al., *Nature Methods*, 2021 | [10.1038/s41592-021-01336-8](https://doi.org/10.1038/s41592-021-01336-8) | `integration_select` (auto-detected from module source) |
| 116 | Luecken et al. et al., *Nature Methods*, 2022 | [10.1038/s41592-021-01336-8](https://doi.org/10.1038/s41592-021-01336-8) | `integration_select` (Atlas-scale integration method comparison; grounds label-free selection metrics and the need for per-dataset method choice.) |
| 117 | singlecell_factory et al., *Unknown*, 2026 | N/A | `integration_select` (Project-local disqualifying-gate rule over label-free discovery metrics (see scripts/bench/integration). Not a published paper; recommendation is opt-in and must be re-validated per cohort.) |
| 118 | Franzen et al. et al., *Database*, 2019 | [10.1093/database/baz046](https://doi.org/10.1093/database/baz046) | `marker_db_loader` (Broad cell type marker resource used as consensus fallback) |
| 119 | Franzén et al., *Database*, 2019 | [10.1093/database/baz046](https://doi.org/10.1093/database/baz046) | `marker_db_loader` (auto-detected from module source) |
| 120 | Hu et al., *Nucleic Acids Research*, 2022 | [10.1093/nar/gkac947](https://doi.org/10.1093/nar/gkac947) | `marker_db_loader` (auto-detected from module source) |
| 121 | Ianevski et al. et al., *Nature Communications*, 2022 | [10.1038/s41467-022-28803-w](https://doi.org/10.1038/s41467-022-28803-w) | `marker_db_loader` (scType marker DB used for condition-specific sub-state detection) |
| 122 | Ianevski et al., *Nature Communications*, 2022 | [10.1038/s41467-022-28803-w](https://doi.org/10.1038/s41467-022-28803-w) | `marker_db_loader` (auto-detected from module source) |
| 123 | Dominguez Conde et al. et al., *Science*, 2022 | [10.1126/science.abq1006](https://doi.org/10.1126/science.abq1006) | `marker_db_loader` (CellTypist model-based marker references) |
| 124 | Hu et al. et al., *Nucleic Acids Research*, 2023 | [10.1093/nar/gkac947](https://doi.org/10.1093/nar/gkac947) | `marker_db_loader` (Curated cell type marker database used for tissue/condition-aware annotation) |
| 125 | Unknown et al., *Unknown*, Unknown | [10.1126/science.abq1006](https://doi.org/10.1126/science.abq1006) | `marker_db_loader` (auto-detected from module source) |
| 126 | Persad et al. et al., *Nature Biotechnology*, 2023 | [10.1038/s41587-023-01716-9](https://doi.org/10.1038/s41587-023-01716-9) | `metacell` (SEACells metacell aggregation; primary backend.) |
| 127 | Persad et al., *Nature Biotechnology*, 2023 | [10.1038/s41587-023-01716-9](https://doi.org/10.1038/s41587-023-01716-9) | `metacell` (auto-detected from module source) |
| 128 | Argelaguet et al. et al., *Genome Biology*, 2020 | [10.1186/s13059-020-02015-1](https://doi.org/10.1186/s13059-020-02015-1) | `multimodal_integration` (MOFA factor model for multi-omics integration.) |
| 129 | Argelaguet et al., *Genome Biology*, 2020 | [10.1186/s13059-020-02015-1](https://doi.org/10.1186/s13059-020-02015-1) | `multimodal_integration` (auto-detected from module source) |
| 130 | Hao et al. et al., *Cell*, 2021 | [10.1016/j.cell.2021.04.048](https://doi.org/10.1016/j.cell.2021.04.048) | `multimodal_integration` (Seurat WNN: weighted nearest neighbours for multimodal joint embeddings.) |
| 131 | Hao et al., *Cell*, 2021 | [10.1016/j.cell.2021.04.048](https://doi.org/10.1016/j.cell.2021.04.048) | `multimodal_integration` (auto-detected from module source) |
| 132 | Sandve et al. et al., *PLOS Computational Biology*, 2013 | [10.1371/journal.pcbi.1003285](https://doi.org/10.1371/journal.pcbi.1003285) | `paper_repro` (Guides provenance capture and reproducibility reporting.) |
| 133 | Sandve et al., *PLoS Computational Biology*, 2013 | [10.1371/journal.pcbi.1003285](https://doi.org/10.1371/journal.pcbi.1003285) | `paper_repro` (auto-detected from module source) |
| 134 | Subramanian et al. et al., *PNAS*, 2005 | [10.1073/pnas.0506580102](https://doi.org/10.1073/pnas.0506580102) | `pathway_analysis` (GSEA methodology. gseapy is the Python port used here.) |
| 135 | Subramanian et al., *Proceedings of the National Academy of Sciences*, 2005 | [10.1073/pnas.0506580102](https://doi.org/10.1073/pnas.0506580102) | `pathway_analysis` (auto-detected from module source) |
| 136 | Schubert et al. et al., *Nature Communications*, 2018 | [10.1038/s41467-017-02391-6](https://doi.org/10.1038/s41467-017-02391-6) | `pathway_analysis` (PROGENy signaling-pathway responsive gene resource consumed via decoupler.) |
| 137 | Schubert et al., *Nature Communications*, 2018 | [10.1038/s41467-017-02391-6](https://doi.org/10.1038/s41467-017-02391-6) | `pathway_analysis` (auto-detected from module source) |
...
| 139 | Pliner et al., *Molecular Cell*, 2018 | [10.1016/j.molcel.2018.06.044](https://doi.org/10.1016/j.molcel.2018.06.044) | `peak_to_gene` (auto-detected from module source) |
| 140 | Ma S, Zhang B, LaFave LM, et al. et al., *Cell*, 2020 | [10.1016/j.cell.2020.09.056](https://doi.org/10.1016/j.cell.2020.09.056) | `peak_to_gene` (SHARE-seq: permutation-based FDR for peak-to-gene linkage significance, controlling block correlation structure without parametric assumptions.) |
| 141 | Ma et al., *Cell*, 2020 | [10.1016/j.cell.2020.09.056](https://doi.org/10.1016/j.cell.2020.09.056) | `peak_to_gene` (auto-detected from module source) |
| 142 | Trevino et al., *Cell*, 2021 | [10.1016/j.cell.2021.07.039](https://doi.org/10.1016/j.cell.2021.07.039) | `peak_to_gene` (auto-detected from module source) |
| 143 | Trevino AE, Müller F, Andersen J, et al. et al., *Cell*, 2021 | [10.1016/j.cell.2021.07.039](https://doi.org/10.1016/j.cell.2021.07.039) | `peak_to_gene` (Methods section 'Peak-to-gene linkages': Pearson correlation between peak accessibility and gene expression in 10x Multiome data as a direct regulatory coupling measure.) |
| 144 | GENCODE consortium et al., *Documentation*, 2024 | [https://www.gencodegenes.org/](https://doi.org/https://www.gencodegenes.org/) | `peak_to_gene` (Canonical TSS coordinates expected for the --gene-tss-bed-path input.) |
| 145 | Stoeckius et al. et al., *Nature Methods*, 2017 | [10.1038/nmeth.4380](https://doi.org/10.1038/nmeth.4380) | `protein_adt` (CITE-seq protocol; CLR normalization for ADT counts.) |
| 146 | Stoeckius et al., *Nature Methods*, 2017 | [10.1038/nmeth.4380](https://doi.org/10.1038/nmeth.4380) | `protein_adt` (auto-detected from module source) |
| 147 | Mulè et al. et al., *Nature Communications*, 2022 | [10.1038/s41467-022-29356-8](https://doi.org/10.1038/s41467-022-29356-8) | `protein_adt` (DSB normalization (stretch-goal stub).) |
| 148 | Mulè et al., *Nature Communications*, 2022 | [10.1038/s41467-022-29356-8](https://doi.org/10.1038/s41467-022-29356-8) | `protein_adt` (auto-detected from module source) |
| 149 | singlecell_factory contributors et al., *Internal documentation*, 2025 | N/A | `pseudo_velocity` (Project-local heuristic: per-cell velocity vectors are computed as a k-nearest-neighbour gradient of dpt_pseudotime in UMAP coordinate space. This is NOT canonical RNA velocity and does not implement the spliced/unspliced dynamical model of La Manno 2018 (Nature) or Bergen 2020 (Nat Biotech, scVelo). It must remain labeled 'proxy' / 'exploratory' in claim ledgers. For canonical RNA velocity claims use workflow.modular.modules.rna_velocity instead, which requires spliced/unspliced count layers.) |
| 150 | Squair et al. et al., *Nature Communications*, 2021 | [10.1038/s41467-021-25960-2](https://doi.org/10.1038/s41467-021-25960-2) | `pseudobulk_de` (Benchmark demonstrating pseudobulk DE outperforms per-cell DE — methodology applied here.) |
| 151 | Squair et al., *Nature Communications*, 2021 | [10.1038/s41467-021-25960-2](https://doi.org/10.1038/s41467-021-25960-2) | `pseudobulk_de` (auto-detected from module source) |
| 152 | Wolf, Angerer, Theis et al., *Genome Biology*, 2018 | [10.1186/s13059-017-1382-0](https://doi.org/10.1186/s13059-017-1382-0) | `qc` (scanpy.pp.calculate_qc_metrics / pp.filter_genes / pp.filter_cells semantics used here.) |
| 153 | Wolf et al., *Genome Biology*, 2018 | [10.1186/s13059-017-1382-0](https://doi.org/10.1186/s13059-017-1382-0) | `qc` (auto-detected from module source) |
| 154 | Luecken, Theis et al., *Molecular Systems Biology*, 2019 | [10.15252/msb.20188746](https://doi.org/10.15252/msb.20188746) | `qc` (Canonical QC threshold guidance (mito %, n_genes/cell, doublet detection).) |
| 155 | Luecken et al., *Molecular Systems Biology*, 2019 | [10.15252/msb.20188746](https://doi.org/10.15252/msb.20188746) | `qc` (auto-detected from module source) |
| 156 | Ingolia et al. et al., *Science*, 2009 | [10.1126/science.1168978](https://doi.org/10.1126/science.1168978) | `ribo_ingest` (Original ribosome profiling (Ribo-seq) methodology defining the footprint count semantics consumed here.) |
| 157 | Ingolia et al., *Science*, 2009 | [10.1126/science.1168978](https://doi.org/10.1126/science.1168978) | `ribo_ingest` (auto-detected from module source) |
| 158 | Grosse et al., *Nature Reviews Molecular Cell Biology*, 2015 | [10.1038/nrm3950](https://doi.org/10.1038/nrm3950) | `ribo_ingest` (auto-detected from module source) |
| 159 | Brar, Weissman et al., *Nature Reviews Molecular Cell Biology*, 2015 | [10.1038/nrm3950](https://doi.org/10.1038/nrm3950) | `ribo_ingest` (Translation efficiency (footprint / RNA) interpretation framework adopted here.) |
| 160 | La Manno et al. et al., *Nature*, 2018 | [10.1038/s41586-018-0414-6](https://doi.org/10.1038/s41586-018-0414-6) | `rna_velocity` (Original spliced/unspliced RNA velocity formulation.) |
| 161 | La Manno et al., *Nature*, 2018 | [10.1038/s41586-018-0414-6](https://doi.org/10.1038/s41586-018-0414-6) | `rna_velocity` (auto-detected from module source) |
| 162 | Bergen et al. et al., *Nature Biotechnology*, 2020 | [10.1038/s41587-020-0591-3](https://doi.org/10.1038/s41587-020-0591-3) | `rna_velocity` (scVelo stochastic + dynamical models. Library used here.) |
| 163 | Bergen et al., *Nature Biotechnology*, 2020 | [10.1038/s41587-020-0591-3](https://doi.org/10.1038/s41587-020-0591-3) | `rna_velocity` (auto-detected from module source) |
| 164 | Granja et al. et al., *Nature Genetics*, 2021 | [10.1038/s41588-021-00790-6](https://doi.org/10.1038/s41588-021-00790-6) | `scatac_pseudobulk_da` (Sample-aware pseudobulk replication for single-cell ATAC-seq accessibility modeling.) |
| 165 | Granja et al., *Nature Genetics*, 2021 | [10.1038/s41588-021-00790-6](https://doi.org/10.1038/s41588-021-00790-6) | `scatac_pseudobulk_da` (auto-detected from module source) |
| 166 | Squair et al. et al., *Nature Communications*, 2021 | [10.1038/s41467-021-25960-2](https://doi.org/10.1038/s41467-021-25960-2) | `scatac_pseudobulk_da` (Sample-level pseudobulk avoids cell-level pseudoreplication.) |
| 167 | Squair et al., *Nature Communications*, 2021 | [10.1038/s41467-021-25960-2](https://doi.org/10.1038/s41467-021-25960-2) | `scatac_pseudobulk_da` (auto-detected from module source) |
| 168 | Chen et al. et al., *Science*, 2015 | [10.1126/science.aaa6090](https://doi.org/10.1126/science.aaa6090) | `spatial_ingest` |
| 169 | Chen et al., *Science*, 2015 | [10.1126/science.aaa6090](https://doi.org/10.1126/science.aaa6090) | `spatial_ingest` (auto-detected from module source) |
| 170 | 10x Genomics et al., *Unknown*, 2020 | [https://www.10xgenomics.com/products/spatial-gene-expression](https://doi.org/https://www.10xgenomics.com/products/spatial-gene-expression) | `spatial_ingest` (Spatial barcoded array; tissue_positions_list.csv schema.) |
| 171 | 10x Genomics et al., *Unknown*, 2023 | [https://www.10xgenomics.com/products/xenium-in-situ](https://doi.org/https://www.10xgenomics.com/products/xenium-in-situ) | `spatial_ingest` (Subcellular in-situ assay; same (x,y) per-cell schema.) |
| 172 | Palla et al. et al., *Nature Methods*, 2022 | [10.1038/s41592-021-01358-2](https://doi.org/10.1038/s41592-021-01358-2) | `spatial_neighborhoods` |
| 173 | Palla et al., *Nature Methods*, 2022 | [10.1038/s41592-021-01358-2](https://doi.org/10.1038/s41592-021-01358-2) | `spatial_neighborhoods` (auto-detected from module source) |
| 174 | Aibar S, González-Blas CB, Moerman T, et al. et al., *Nature Methods*, 2017 | [10.1038/nmeth.4463](https://doi.org/10.1038/nmeth.4463) | `tf_network` (TF → gene network inference framework; this module implements a chromVAR-anchored variant that requires both motif accessibility and peak-to-gene support.) |
| 175 | Aibar et al., *Nature Methods*, 2017 | [10.1038/nmeth.4463](https://doi.org/10.1038/nmeth.4463) | `tf_network` (auto-detected from module source) |
| 176 | Trevino et al., *Cell*, 2021 | [10.1016/j.cell.2021.07.039](https://doi.org/10.1016/j.cell.2021.07.039) | `tf_network` (auto-detected from module source) |
| 177 | Trevino AE, Müller F, Andersen J, et al. et al., *Cell*, 2021 | [10.1016/j.cell.2021.07.039](https://doi.org/10.1016/j.cell.2021.07.039) | `tf_network` (Source paper for F-6 TF-gene network reproduction (Wave-5.6 MV3).) |
| 178 | Haghverdi, Buttner, Wolf et al. et al., *Nature Methods*, 2016 | [10.1038/nmeth.3971](https://doi.org/10.1038/nmeth.3971) | `trajectory` (Diffusion pseudotime (sc.tl.dpt) used for ordering cells along trajectories.) |
| 179 | Haghverdi et al., *Nature Methods*, 2016 | [10.1038/nmeth.3971](https://doi.org/10.1038/nmeth.3971) | `trajectory` (auto-detected from module source) |
| 180 | Setty et al. et al., *Nature Biotechnology*, 2019 | [10.1038/s41587-019-0068-4](https://doi.org/10.1038/s41587-019-0068-4) | `trajectory` (Pseudotime computed over a joint manifold embedding (X_wnn or X_pca). Supports the rationale for running DPT on the best available joint embedding rather than UMAP.) |
| 181 | Setty et al., *Nature Biotechnology*, 2019 | [10.1038/s41587-019-0068-4](https://doi.org/10.1038/s41587-019-0068-4) | `trajectory` (auto-detected from module source) |
| 182 | Wolf et al. et al., *Genome Biology*, 2019 | [10.1186/s13059-019-1663-x](https://doi.org/10.1186/s13059-019-1663-x) | `trajectory` (PAGA topology-preserving abstraction. sc.tl.paga used here.) |
| 183 | Wolf et al., *Genome Biology*, 2019 | [10.1186/s13059-019-1663-x](https://doi.org/10.1186/s13059-019-1663-x) | `trajectory` (auto-detected from module source) |
| 184 | Hao et al. et al., *Cell*, 2021 | [10.1016/j.cell.2021.04.048](https://doi.org/10.1016/j.cell.2021.04.048) | `trajectory` (WNN (Weighted Nearest Neighbor) joint embedding. X_wnn is preferred over X_pca for pseudotime when available, as it captures multi-modal cell state more faithfully.) |
| 185 | Hao et al., *Cell*, 2021 | [10.1016/j.cell.2021.04.048](https://doi.org/10.1016/j.cell.2021.04.048) | `trajectory` (auto-detected from module source) |
| 186 | Tirosh et al. et al., *Science*, 2016 | [10.1126/science.aad0501](https://doi.org/10.1126/science.aad0501) | `tumor_microenvironment` (Score-gene-set framework used for TME marker scoring.) |
| 187 | Tirosh et al., *Science*, 2016 | [10.1126/science.aad0501](https://doi.org/10.1126/science.aad0501) | `tumor_microenvironment` (auto-detected from module source) |
| 188 | Aran et al. et al., *Nature Immunology*, 2019 | [10.1038/s41590-018-0276-y](https://doi.org/10.1038/s41590-018-0276-y) | `tumor_microenvironment` (Reference-based lung TME phenotype assignments.) |
| 189 | Aran et al., *Nature Immunology*, 2019 | [10.1038/s41590-018-0276-y](https://doi.org/10.1038/s41590-018-0276-y) | `tumor_microenvironment` (auto-detected from module source) |
| 190 | Cerami et al. et al., *Cancer Discovery*, 2012 | [10.1158/2159-8290.CD-12-0095](https://doi.org/10.1158/2159-8290.CD-12-0095) | `validate_cbioportal` (External cancer genomics validation portal queried by this module.) |
| 191 | Cerami et al., *Cancer Discovery*, 2012 | [10.1158/2159-8290.CD-12-0095](https://doi.org/10.1158/2159-8290.CD-12-0095) | `validate_cbioportal` (auto-detected from module source) |
| 192 | Bagaev et al. et al., *PLoS Computational Biology*, 2015 | [10.1371/journal.pcbi.1004503](https://doi.org/10.1371/journal.pcbi.1004503) | `vdj_ingest` (Canonical VDJ repertoire analysis conventions adopted here for clonotype definition (V-gene + J-gene + CDR3 nt).) |
| 193 | Shugay et al., *PLOS Computational Biology*, 2015 | [10.1371/journal.pcbi.1004503](https://doi.org/10.1371/journal.pcbi.1004503) | `vdj_ingest` (auto-detected from module source) |
...
| 195 | Sturm et al., *Bioinformatics*, 2020 | [10.1093/bioinformatics/btaa611](https://doi.org/10.1093/bioinformatics/btaa611) | `vdj_ingest` (auto-detected from module source) |
| 196 | 10x Genomics et al., *Software documentation*, 2024 | [https://support.10xgenomics.com/single-cell-vdj/software](https://doi.org/https://support.10xgenomics.com/single-cell-vdj/software) | `vdj_ingest` (Authoritative spec for filtered_contig_annotations.csv / clonotypes.csv schemas consumed here.) |
| 197 | Gini et al., *Studi Economico-Giuridici della Universita de Cagliari*, 1912 | [https://en.wikipedia.org/wiki/Gini_coefficient](https://doi.org/https://en.wikipedia.org/wiki/Gini_coefficient) | `vdj_metrics` (Gini coefficient used to quantify clonotype abundance inequality (1 - sum_i p_i (2 i - n - 1)/n).) |
| 198 | Shannon et al., *Bell System Technical Journal*, 1948 | [10.1002/j.1538-7305.1948.tb01338.x](https://doi.org/10.1002/j.1538-7305.1948.tb01338.x) | `vdj_metrics` (Shannon entropy used here for clonotype diversity per sample.) |
| 199 | Shannon et al., *Bell System Technical Journal*, 1948 | [10.1002/j.1538-7305.1948.tb01338.x](https://doi.org/10.1002/j.1538-7305.1948.tb01338.x) | `vdj_metrics` (auto-detected from module source) |
| 200 | Bagaev et al. et al., *PLoS Computational Biology*, 2015 | [10.1371/journal.pcbi.1004503](https://doi.org/10.1371/journal.pcbi.1004503) | `vdj_metrics` (Canonical repertoire diversity conventions adopted here (Shannon / Gini per sample; clonal expansion buckets).) |
| 201 | Shugay et al., *PLOS Computational Biology*, 2015 | [10.1371/journal.pcbi.1004503](https://doi.org/10.1371/journal.pcbi.1004503) | `vdj_metrics` (auto-detected from module source) |
---
...
Last refreshed: `2026-08-23T06:00:50Z`
