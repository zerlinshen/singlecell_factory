"""Wave-5.5 public-matrix ATAC transferred pseudotime analysis (Figure 3B).

# STATUS: one-off, promote-on-reuse

Performs the cross-modality pseudotime transfer and ATAC embedding computation:
- Reads RNA velocity reference h5ad (velocity_pseudotime pre-computed upstream)
- Reads public ATAC gene-activity matrix from GSE162170
- Transfers RNA velocity pseudotime to ATAC cells via cosine kNN in shared z-scored
  gene activity / expression space (bounded, auditable cross-modality transfer)
- Computes a reproducible public-matrix ATAC embedding (scale -> PCA -> neighbors -> UMAP)
- Writes figure3b_atac_transferred_pseudotime_subset.h5ad with:
    obs["transferred_pseudotime"], obs["mean_transfer_cosine_distance"],
    obs["transferred_rna_cell_type"] (if cell_type present in RNA ref)
    obsm["X_pca"], obsm["X_umap"]
- Writes figure3b_atac_pseudotime_summary.json with full provenance

Boundary: public processed-matrix reproduction, not FASTQ/fragments-level
scATAC reprocessing and not the original author's exact integration model.

Rendering of figures from these outputs is done by:
  plotting_factory/python/wave5/wave5_render_public_atac_pseudotime_figure3b.py
"""
...
def read_atac_gene_activity(
    path: Path, wanted_genes: Iterable[str]
...
def choose_atac_cells(meta: pd.DataFrame, max_cells: int, seed: int) -> list[str]:
    ...
    for _, idx in sorted(groups.items(), key=lambda kv: str(kv[0])):
        ...
        chosen.extend(
            meta.index[rng.choice(idx, size=take, replace=False)].astype(str).tolist()
        )
    if len(chosen) > max_cells:
        chosen = rng.choice(np.asarray(chosen), size=max_cells, replace=False).astype(
            str
        ).tolist()
    elif len(chosen) < max_cells:
        remaining = np.setdiff1d(
            meta.index.astype(str), np.asarray(chosen), assume_unique=False
        )
        if len(remaining):
            chosen.extend(
                rng.choice(
                    remaining,
                    size=min(max_cells - len(chosen), len(remaining)),
                    replace=False,
                ).astype(str).tolist()
            )
    return chosen
...
def marker_direction_metrics(
    adata: ad.AnnData, genes: list[str]
    ...
    for gene in genes:
        ...
        rho, pval = (
            spearmanr(pt[valid], expr[valid]) if valid.sum() >= 10 else (np.nan, np.nan)
        )
        metrics[gene] = {
            "present": True,
    ...
    for gene in EXPECTED_NEGATIVE:
        ...
        checks.append(
            {"marker": gene, "expected": "negative", "rho": rho, "pass": rho is not None and rho < 0}
        )
    for gene in EXPECTED_POSITIVE:
        rho = metrics.get(gene, {}).get("spearman_rho")
        checks.append(
            {"marker": gene, "expected": "positive", "rho": rho, "pass": rho is not None and rho > 0}
        )
...
def main() -> int:
    ...
    atac_cells, atac_genes, atac_matrix, atac_missing = read_atac_gene_activity(
        args.atac_gene_activity, marker_genes
    )
    meta = pd.read_csv(
        args.atac_metadata, sep="\t", compression="gzip", dtype=str
    ).set_index("Cell.ID")
    ...
    pt = pt[finite]
    rna_cell_types = (
        rna.obs.loc[finite, "cell_type"].astype(str).to_numpy()
    ...
    # Compute reproducible public-matrix ATAC embedding from selected gene activities.
    ...
    summary = {
        ...
        "truth_boundary": (
            "Public processed-matrix ATAC gene-activity transfer from 3A RNA velocity "
            "pseudotime; not FASTQ/fragments-level and not the original author integration model."
        ),
        ...
        "parameters": {
            ...
            "seed": args.seed,
            "feature_scaling": (
                "within-modality z-score on log1p ATAC gene activity and RNA reference expression"
            ),
