...
- **Ledger note (2026-05-27, W11 — scVI stale-counts contract):** Before this fix, `_run_scvi` always preferred `layers['counts']` (the raw pre-ambient UMI matrix written by `cellranger.py`). When `ambient_correction` fired, it overwrote `adata.X` with DecontX-corrected counts and snapshotted the pre-correction raw to `layers['counts_raw_pre_decontx']` but did **not** refresh `layers['counts']`. Consequence: scVI trained on **uncorrected** counts while clustering and DE operated on the **ambient-corrected** matrix — a silent cross-module inconsistency. **Any prior run where ambient_correction triggered AND scVI integration ran used the stale contract; its scVI/integration recommendation is NOT comparable to runs under the fixed contract and must be regenerated before cross-run comparison.** The fix makes `_run_scvi` consume the ambient-corrected signal when ambient ran (matching the clustering/DE path). **Because DecontX corrected counts are FRACTIONAL and scVI's NB/ZINB likelihood requires INTEGER counts, the corrected matrix is rounded to integers into `layers['counts_decontx_int']` for scVI** (contract `scvi-counts-v3-ambient-corrected-rounded-int`); a real integrality guard now fails loud if any non-integer matrix reaches scVI. `ctx.metadata['scvi_consumed_counts_source']`, `['scvi_counts_contract']`, and `['scvi_corrected_counts_rounded_to_int']` record the consumed counts, contract version, and whether rounding was applied. Runs where ambient did not trigger are unaffected (scVI still uses `layers['counts']`).
