...
- **Device contract**: Reference mapping defaults to `auto`, which selects exactly one backend before mapping and never dual-runs production data. Only an exact current promoted certificate can route to GPU. The `trevino-fetal-cortex-v1` certificate is revoked after its independently query-blind validation failed, so `auto` routes this domain directly to CPU and explicit `--reference-device gpu` fails closed. Missing/unknown certificates, `--gpu-mode off`, unavailable CUDA/cuML, or backend-version drift also route `auto` directly to CPU. CPU is conservative fallback behavior, not biological ground truth.
...
The benchmark is a bounded coordinator with isolated private CPU/GPU child processes. It freezes source identity, split, matrices, cell order, genes, parameters, and the full-precision threshold before either child starts. Each child warms exactly 256 query rows once, makes three measured repetitions, records its own `ru_maxrss`, and emits a failure receipt on any integrity or GPU failure. Exact user-consumed scientific columns are required across repetitions; mean-neighbor distance must satisfy the predeclared `rtol=atol=1e-6` float contract. Raw file hashes remain visible evidence but are not substituted for scientific-output comparison. A GPU failure is `FAIL_NOT_PROMOTED`, not a CPU fallback.

The historical replacement `/home/zerlinshen/projects/reference-atlas-ood-validation/runs/2026-08-22T1558Z-c38053d/` passed its then-declared gates and measured `11.04x`, but its source-wide retained-feature mask was not independently query-blind. It is superseded technical history and must not authorize routing. The corrected reference-only run `/home/zerlinshen/projects/reference-atlas-ood-validation/runs/2026-08-23T1054Z-0b66858/` failed all four OOD gates (known coverage `0.5127`, known all-cell macro-F1 `0.5749`, Microglia rejection recall `0.4305`, rejection-rate delta `-0.0568`). CPU was exactly repeatable; GPU changed one confidence value across repetitions and failed the lane. The certificate is revoked and `auto` routes this domain directly to CPU. Do not infer that CPU is biological truth: both lanes share a method that failed the query-blind OOD gates.
